...
function pushHistoryState(level, data) {
  ...
  history.pushState({ level, ...data }, '');
}

function pushModalState() {
  if (isPopState) return;
  const current = history.state || { level: 0 };
  history.pushState({ ...current, modalOpen: true }, '');
}
...
def repl(m):
    var_name = m.group(1)
    return f"{m.group(0)} if (typeof pushModalState === 'function' && {var_name}.classList && {var_name}.classList.contains('modal-screen')) pushModalState();"
